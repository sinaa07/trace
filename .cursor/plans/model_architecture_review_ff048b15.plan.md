...
Current repo context: Phase 1–2 already has affine clock correction (`[backend/app/core/temporal/engine.py](backend/app/core/temporal/engine.py)`), YAML rule anomalies including signal transitions (`[backend/app/core/anomalies/rules/railway_rules.yaml](backend/app/core/anomalies/rules/railway_rules.yaml)`), and conflict detection. Phase 3 agents/MCP and Phase 4 Neo4j are not built yet. Blueprint (`[docs/_blueprint_extract.txt](docs/_blueprint_extract.txt)`) scopes PC/GES to **Future**, BN/DoWhy/DS/DTW to **Advanced**, and requires local/free LLM fallback.
...
## Priority fixes to the written research doc
...
No code changes in this review step — this is an architecture decision record for next-phase planning.